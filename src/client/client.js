...
(function (Status1) {
  ...
  Status1[Status1["RequestedRangeNotSatisfiable"] = 416] =
  ...
  Status1[Status1["RequestHeaderFieldsTooLarge"] = 431] =
    "RequestHeaderFieldsTooLarge";
  Status1[Status1["UnavailableForLegalReasons"] = 451] =
  ...
  Status1[Status1["NetworkAuthenticationRequired"] = 511] =
...
class TextProtoReader {
  ...
  async readMIMEHeader() {
    ...
    if (buf === null) {
      ...
      throw new Deno.errors.InvalidData(
        `malformed MIME header initial line: ${str1(line)}`,
      );
    }
    while (true) {
      ...
      if (i < 0) {
        throw new Deno.errors.InvalidData(
          `malformed MIME header line: ${str1(kv)}`,
        );
      }
      ...
      i++;
      while (
        i < kv.byteLength && (kv[i] == charCode(" ") || kv[i] == charCode("\t"))
      ) {
        i++;
      }
      const value = str1(kv.subarray(i)).replace(
...
class BufReader {
  ...
  async readLine() {
    ...
    try {
      ...
      if (
        !this.eof && partial2.byteLength > 0 &&
        partial2[partial2.byteLength - 1] === CR
      ) {
...
class TextProtoReader1 {
  ...
  async readMIMEHeader() {
    ...
    while (true) {
      ...
      while (
        i < kv.byteLength &&
        (kv[i] == charCode1(" ") || kv[i] == charCode1("\t"))
      ) {
        i++;
      }
      const value = str(kv.subarray(i)).replace(
        invalidHeaderCharRegex1,
        encodeURI,
      );
      try {
        m.append(key, value);
...
class BufReader1 {
  ...
  async readLine() {
    ...
    try {
      ...
      if (
        !this.eof && partial2.byteLength > 0 &&
        partial2[partial2.byteLength - 1] === CR1
      ) {
...
class BufWriterSync1 extends AbstractBufBase1 {
  static create(writer, size = 4096) {
    return writer instanceof BufWriterSync1
      ? writer
      : new BufWriterSync1(writer, size);
  }
    ...
    if (!this._body) {
      if (this.contentLength != null) {
        ...
        if (transferEncoding != null) {
          const parts = transferEncoding.split(",").map((e) =>
            e.trim().toLowerCase()
          );
          assert(

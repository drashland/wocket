...
Rhum.testPlan("tests/unit/client/client_test.ts", () => {
  Rhum.testSuite("connect()", () => {
    Rhum.testCase(
      "It should throw an error when no channels are passed in",
      async () => {
        const server = new Server();
        await server.run(configs);
        ...
        Rhum.asserts.assertEquals(
          msg,
          "You must specify channels to connect to.",
        );
      },
    );
    Rhum.testCase(
      "It should throw an error when passed in channel(s) do not exist",
      async () => {
        const server = new Server();
        await server.run(configs);
        const c = new WocketClient();
        ...
        Rhum.asserts.assertEquals(
          msg,
          'Channel "idontexist" does not exist. You must open this on your server.',
        );
      },
    );
    ...
    Rhum.testCase(
      "It should set the hostname to 'localhost' by default",
      async () => {
        const server = new Server();
        await server.run(configs);
        server.on("chat", (packet) => {});
    ...
    Rhum.testCase(
      "It should set the protocol to  'ws' by default",
      async () => {
        const server = new Server();
        await server.run(configs);
        server.on("chat", (packet) => {});
  ...
  Rhum.testSuite("on()", () => {
    Rhum.testCase(
      "Should register a listener and handle events for that channel",
      async () => {
        const server = new Server();
        await server.run(configs);
        server.on("chat", (packet) => {});
  ...
  Rhum.testSuite("to()", () => {
    Rhum.testCase(
      "It should send a message to the socket server when connected to the channel",
      async () => {
        const server = new Server();
        await server.run(configs);
        const p = deferred();

...
import { Channel, Server, Client } from "../../mod.ts";
...
Rhum.testPlan("unit/channel_test.ts", () => {
  ...
  Rhum.testSuite("close()", () => {
    Rhum.testCase("Will send a close event to every client connected to the specific channel", async () => {
  ...
  Rhum.testSuite("executeCallbacks()", () => {
    Rhum.testSuite("Should call every callback defined in the `callbacks` property of the `Channel` class", async () => {
      const channel = new Channel("chat")
  ...
  Rhum.testSuite("hasClient()", () => {
    ...
    Rhum.testCase("Will return false of the channel does not have that client", () => {

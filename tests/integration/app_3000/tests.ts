...
async function waitForMessage(client: WebSocket): Promise<Record<string, unknown>> {
...
async function sendMsgAndGetRes(client: WebSocket, message: any, channel: string) {
  ...
  Rhum.testSuite("server", () => {
    ...
    Rhum.testCase(
      ...
      async () => {
        ...
        const clientOne = await createClient(["emit to specific client", "get-id"])
        const clientTwo = await createClient(["emit to specific client", "get-id"])
        const clientThree = await createClient(["emit to specific client", "get-id"])
        ...
        const client3Id = (await sendMsgAndGetRes(clientThree, {}, "get-id")).id
      ...
      async () => {
        ...
        client.onmessage = function (message) {
          Rhum.asserts.assertEquals(
            message.data.includes(" is not connected to chan1"), true
          );
          client.close();
        };
        client.onclose = function () {
      ...
      async () => {
        ...
        client.onmessage = function (message) {
          ...
          if (messageCount === 1) {
            ...
            Rhum.asserts.assertEquals(
              message.data.includes(" is not connected to chan2"),true
            );
            client.close();
          }
        };
